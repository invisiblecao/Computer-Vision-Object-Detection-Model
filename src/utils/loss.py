...
class yololoss(nn.Module):
    ...
    def forward(self, prediction, target):
        ...
        box_target_not_response_iou = box_target[coo_not_response_mask].view(-1, 5)
        box_target_not_response_iou[:, 4] = 0
        not_response_loss = F.mse_loss(box_pred_not_response[:, 4], box_target_not_response_iou[:, 4], reduction='sum')
        ##################################################################
